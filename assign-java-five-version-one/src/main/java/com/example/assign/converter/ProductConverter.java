...
public class ProductConverter {
    ...
    public List<Product> toListEntity(List<ProductDTO> dtos) {
        return Optional.of(dtos).map(products -> List.of(mapper.map(dtos, Product[].class))).orElse(null);
    }

    public List<ProductDTO> toListDTO(List<Product> entities) {
        return Optional.of(entities).map(products -> List.of(mapper.map(entities, ProductDTO[].class))).orElse(null);
    }
}
